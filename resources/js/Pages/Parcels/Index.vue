  ...
  <AppLayout>
    ...
    <template #content>
      <v-container fluid>
        ...
        <v-card 
          elevation="2"
          style="border-radius: 16px; background: white; overflow: hidden;"
        >
          <v-card-title class="pa-6" style="background: linear-gradient(135deg, #f8f9fa 0%, #e9ecef 100%); border-bottom: 1px solid #dee2e6;">
            <div class="d-flex align-center">
              <v-avatar size="40" class="mr-3" style="background: linear-gradient(135deg, #1976d2 0%, #1565c0 100%);">
                <v-icon color="white" size="20">mdi-package-variant</v-icon>
              </v-avatar>
              <div>
                <h2 class="text-h5 font-weight-bold mb-1" style="color: #1a1d29;">{{ $t('parcels.title') }}</h2>
                <p class="text-body-2 mb-0" style="color: #6b7280;">{{ parcels.total }} total parcels</p>
              </div>
            </div>
            <v-spacer></v-spacer>
            <v-chip 
              ...
              class="mr-2"
            >
              {{ statusOptions.find(s => s.value === filters.status)?.text }}
            </v-chip>
            <v-chip 
              ...
              size="small"
            >
              {{ companyOptions.find(c => c.value === filters.company_id)?.text }}
            </v-chip>
          </v-card-title>
          ...
          <v-card-text class="pa-0">
            <div v-if="loading" class="text-center pa-8">
              ...
              <p class="text-body-1 mt-4" style="color: #6b7280;">Loading parcels...</p>
            </div>
            
            <div v-else-if="parcels.data.length === 0" class="text-center pa-8">
              <v-icon size="64" color="grey-lighten-2">mdi-package-variant-closed</v-icon>
              <h3 class="text-h6 font-weight-medium mt-4 mb-2" style="color: #374151;">No parcels found</h3>
              <p class="text-body-2" style="color: #6b7280;">Try adjusting your filters or create a new parcel</p>
            </div>
            ...
            <div v-else>
              ...
              <div class="parcel-cards-container">
                <v-card
                  ...
                  elevation="1"
                  style="border-radius: 12px; border: 1px solid #e5e7eb; transition: all 0.3s ease; cursor: pointer;"
                  @click="openTrackingPage(parcel.tracking_number, parcel.id)"
                >
                    ...
                    <v-row>
                      ...
                      <v-col cols="12" md="3">
                        <div class="d-flex align-center mb-3">
                          <v-avatar size="32" class="mr-3" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);">
                            <v-icon color="white" size="16">mdi-barcode</v-icon>
                          </v-avatar>
                          <div class="flex-grow-1">
                            <div class="d-flex align-center">
                              <v-icon 
                                v-if="lastClickedParcelId === parcel.id"
                                color="success" 
                                size="20" 
                              ...
                              <div class="text-h6 font-weight-bold" style="color: #1a1d29;">{{ parcel.tracking_number }}</div>
                            ...
                            <div class="d-flex flex-column">
                              <v-chip
                                ...
                                class="mt-1 w-fit"
                              >
                                {{ $t(`parcels.status_${parcel.status}`) }}
                              </v-chip>
                              
                              <!-- Pending days count for urgency -->
                              <div 
                                v-if="parcel.status === 'pending' && getPendingDays(parcel) > 0"
                                class="mt-1"
                              >
                                <v-chip
                                  :color="getUrgencyColor(parcel)"
                                  size="x-small"
                                  variant="outlined"
                                  class="urgency-chip"
                                >
                                  <v-icon left size="12">mdi-clock-alert</v-icon>
                                  {{ getPendingDays(parcel) }} {{ getPendingDays(parcel) === 1 ? 'day' : 'days' }} pending
                                </v-chip>
                              </div>
                            </div>
                          </div>
                      ...
                      <v-col cols="12" md="4">
                        <div class="recipient-info">
                          <h4 class="text-subtitle-1 font-weight-bold mb-2" style="color: #1a1d29;">
                            <v-icon size="18" class="mr-1" style="color: #667eea;">mdi-account</v-icon>
                          ...
                          <div class="mb-2">
                            <v-btn
                              ...
                              size="small"
                              color="primary"
                              class="pa-0 text-left justify-start"
                          ...
                          <div v-if="parcel.state" class="mb-2">
                            <v-chip
                              color="info"
                              variant="outlined"
                              size="small"
                              class="mr-2"
                            >
                              <v-icon left size="14">mdi-map</v-icon>
                              {{ parcel.state.name }}
                            </v-chip>
                            <!-- <v-chip
                              v-if="parcel.city"
                              ...
                              size="small"
                            >
                              <v-icon left size="14">mdi-city</v-icon>
                              {{ parcel.city.name }}
                            </v-chip> -->
                          </div>
                      ...
                      <v-col cols="12" md="3">
                        <div class="financial-info">
                          <div v-if="parcel.cod_amount" class="mb-3">
                            <div class="text-caption" style="color: #6b7280;">COD Amount</div>
                            <div class="text-h6 font-weight-bold" style="color: #059669;">
                              {{ formatCurrency(parcel.cod_amount) }}
                            </div>
                          </div>
                          
                          <!-- Description -->
                          <div v-if="parcel.description" class="mb-2">
                            <div class="text-caption" style="color: #6b7280;">
                              <v-icon size="12" class="mr-1">mdi-text</v-icon>
                              Description
                            </div>
                            <div class="text-body-2" style="color: #374151; line-height: 1.4;">
                              {{ parcel.description }}
                            </div>
                          </div>
                          
                          <!-- Notes -->
                          <div v-if="parcel.notes" class="mb-2">
                            <div class="text-caption" style="color: #6b7280;">
                          ...
                          <div v-if="parcel.company" class="mb-2">
                            <v-chip
                              color="primary"
                              variant="outlined"
                              size="small"
                            >
                              <v-icon left size="14">mdi-domain</v-icon>
                              {{ parcel.company.name }}
                            </v-chip>
                          </div>
                        </div>
              ...
              <!-- Pagination -->
              <div class="pa-4" style="border-top: 1px solid #e5e7eb; background: #f8f9fa;">
                <v-pagination
                  v-model="currentPage"
                  :length="totalPages"
                  :total-visible="7"
...
export default {
  ...
  data() {
    return {
      loading: false,
      searchTimeout: null,
      currentPage: this.parcels?.current_page || 1,
