    ...
    <AppLayout>
        ...
        <v-dialog v-model="showPaymentModal" max-width="500px">
            <v-card style="border-radius: 12px">
                ...
                <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn @click="closePaymentDialog" color="grey">{{
                        $t("common.cancel")
                    }}</v-btn>
                    <v-btn @click="markAsPaid" color="success">{{
                        $t("expenses.mark_as_paid")
                    }}</v-btn>
                </v-card-actions>
            </v-card>
        ...
        <v-dialog v-model="showDeleteModal" max-width="400px">
            <v-card style="border-radius: 12px">
                ...
                <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn @click="closeDeleteDialog" color="grey">{{
                        $t("common.cancel")
                    }}</v-btn>
                    <v-btn @click="deleteExpense" color="error">{{
                        $t("common.delete")
                    }}</v-btn>
                </v-card-actions>
            </v-card>
...
export default {
    ...
    data() {
        return {
            ...
            statusOptions: [
                ...
                {
                    title: this.$t("expenses.status.approved"),
                    value: "approved",
                },
                { title: this.$t("expenses.status.paid"), value: "paid" },
                {
                    title: this.$t("expenses.status.rejected"),
                    value: "rejected",
                },
            ],
            paymentMethods: [
                {
                    title: this.$t("expenses.payment_methods.cash"),
                    value: "cash",
                },
                {
                    title: this.$t("expenses.payment_methods.bank_transfer"),
                    value: "bank_transfer",
                },
                {
                    title: this.$t("expenses.payment_methods.check"),
                    value: "check",
                },
                {
                    title: this.$t("expenses.payment_methods.card"),
                    value: "card",
                },
            ],
        };
    ...
    methods: {
        ...
        markAsPaid() {
            router.post(
                route("expenses.mark-as-paid", this.selectedExpense.id),
                this.paymentForm,
                {
                    onSuccess: () => {
