...
class ParcelController extends Controller
{
    ...
    public function store(Request $request)
    {
        $validated = $request->validate([
            "tracking_number" =>
                "required|string|unique:parcels,tracking_number",
    ...
    public function edit(Parcel $parcel)
    {
        ...
        return Inertia::render("Parcels/Edit", [
            ...
            "drivers" => Driver::active()->get(),
            "states" => State::active()->get(),
            "cities" => City::where("state_id", $parcel->state_id)
                ->active()
                ->get(),
        ]);
    }
    ...
    public function update(Request $request, Parcel $parcel)
    {
        $validated = $request->validate([
            "tracking_number" =>
                "required|string|unique:parcels,tracking_number," . $parcel->id,
            "company_id" => "required|exists:companies,id",
            "recipient_name" => "required|string|max:255",
            "recipient_phone" => "required|string|max:20",
            "recipient_address" => "required|string",
            ...
            "delivery_fee" => "nullable|numeric|min:0",
            "status" =>
                "required|in:pending,assigned,dispatched,delivered,returned,failed",
            "notes" => "nullable|string",
            "assigned_driver_id" => "nullable|exists:drivers,id",
        ]);
    ...
    public function importExcel(Request $request)
    {
        ...
        try {
            ...
            Log::error(
                "Unexpected error during Excel import: " . $e->getMessage(),
                [
                    "exception" => $e->getTraceAsString(),
                    "line" => $e->getLine(),
                    "file_path" => $e->getFile(),
    ...
    public function stopDeskPayment(Request $request)
    {
        $recentCollections = \App\Models\Collection::with(["parcel"])
            ...
            ->map(function ($collection) {
                return [
                    ...
                    "changeAmount" =>
                        $collection->amount_given -
                        ($collection->parcel->cod_amount ?? 0),
        ...
        if ($user->role === "admin") {
            ...
            $activeCases = \App\Models\MoneyCase::where("status", "active")
                ->where(function ($query) use ($currentUserId) {
                    $query
                        ->whereNull("last_active_by") // Free cases
        ...
        return Inertia::render("StopDeskPayment/Index", [
            ...
            "userLastActiveCaseId" => $userLastActiveCase
                ? $userLastActiveCase->id
                : null,
            // 'auth' => [
            //     'user' => [
            //         'id' => auth()->user()->id,
    ...
    public function createManualParcelAndCollect(Request $request)
    {
        ...
        try {
            ...
            if ($request->case_id) {
                // Get and activate the money case for the user
                $moneyCase = \App\Models\MoneyCase::findOrFail(
                    $request->case_id,
                );
            ...
            $collection = Collection::create([
                ...
                "created_by" => auth()->id(),
                "note" =>
                    "Manual parcel entry and stop desk payment collection",
            ...
            return response()->json([
                "success" => true,
                "message" =>
                    "Manual parcel created and payment confirmed successfully",
                "parcel_id" => $parcel->id,
                "change_amount" => $changeAmount,
                "collection" => [
                    "id" => $collection->id,
                    "collected_at" => $collection->collected_at->format(
                        "Y-m-d H:i:s",
                    ),
            ...
            return back()->with([
                "paymentResult" => [
                    "success" => false,
                    "message" =>
                        "Failed to create manual parcel: " . $e->getMessage(),
                ],
            ]);
        }
    }
